...
function Sidebar(editorUi, container)
{
    ...
    this.palettesExpandHandlers = new Object();
...
function getLocalizedTitle(title){
	var key='sidebar'+title.replace(/\s+/g, ''); //Removes spaces
	
	var localizedTitle=mxResources.get(key);
...
{	
    ...
    this.addFoldingHandler(id, elt, div, onInit);
 ...
 */
Sidebar.prototype.addFoldingHandler = function(paletteId, title, content, funct)
{
	...
	this.palettesExpandHandlers[paletteId] = function(evt, expand)
    {
        if (content.style.display == 'none' && (evt != null || expand))
        {
			if (!initialized)
			{
				initialized = true;
        ...
        else if (content.style.display != 'none' && (evt != null || !expand))
        {
	...
	mxEvent.addListener(title, 'click', this.palettesExpandHandlers[paletteId]);
};
