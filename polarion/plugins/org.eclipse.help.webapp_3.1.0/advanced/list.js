...
var isMozilla = navigator.userAgent.indexOf('Firefox') != -1;
...
function selectTopicById(id)
{
	if(id!=''){
		...
		var topic = document.getElementById('a0');
		if (topic)
		{			
			var newurl= window.top.location.href;			
			
			var j = newurl.lastIndexOf("topic=");
			if(j>0)
			    return false;		
			
			var topic_url= topic.href;
			var k = topic_url.lastIndexOf("resultof=");
			if(k>0)
			    return false;
			
			var i = topic_url.lastIndexOf("/");
			newurl = newurl + "&topic=%2Fcom.polarion.xray.doc.user%2F"+topic_url.substring(i+1);
