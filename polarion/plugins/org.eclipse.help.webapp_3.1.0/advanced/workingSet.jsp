...
var oldName = '<%=UrlUtil.JavaScriptEncode(data.isEditMode()? data.getWorkingSetName():"")%>';
...
function doSubmit()
{
	try
	{
		var op = '<%=UrlUtil.JavaScriptEncode(data.getOperation())%>';
		...
		var query = "operation="+encodeURIComponent(op)+"&workingSet="+encodeURIComponent(workingSet)+ hrefs+"&oldName="+encodeURIComponent(oldName);
...
<form onsubmit="doSubmit();return false;">
	<table id="wsTable" width="100%" cellspacing=0 cellpading=0 border=0 align=center >
		...
		<tr><td style="padding:0px 10px;"><input type="text" id="workingSet" name="workingSet" value="<%=UrlUtil.htmlEncode(data.isEditMode()?data.getWorkingSetName():"")%>" maxlength=256 alt='<%=ServletResources.getString("WorkingSetName", request)%>' title='<%=ServletResources.getString("WorkingSetName", request)%>' onkeyup="enableOK();return true;">
        </td></tr>
