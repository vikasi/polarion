 ...
 */
  ...
  sizeDivs: function() {
    ...
    //this.cancelMenu();
    this.unhighlight();
    el = document.getElementById('SpaceIndex_tab');
    if (el && (this.pageSize > 1))
    {                         
//alert("offsetHeight: " + this.tbody[1].offsetHeight);
//alert(parseInt(el.style.height));
//       this.tbody[1].offsetHeight = parseInt(el.style.height) - 20;  
//alert("New offsetHeight: " + this.tbody[1].offsetHeight);
    }

    this.baseSizeDivs();
    if (this.pageSize == 0) return;
//alert('this.resizeDiv ' + this.pageSize + ' id ' + this.tableId + ' dht ' + this.dataHt );
//    if (this.pageSize > 1)
//      this.dataHt = this.dataHt - 36;


    this.rowHeight = Math.floor(this.dataHt/this.pageSize);
...
//alert(this.scrWi);
    ...
    this.setHorizontalScroll();
 //   $(this.tableId+'_outerDiv').setStyle({width:($(this.tableId+'_outerDiv').getWidth()-this.options.scrollBarWidth)+'px'});
//    $(this.tableId+'_frozenTabsDiv').setStyle({width:($(this.tableId+'_frozenTabsDiv').getWidth()-this.options.scrollBarWidth)+'px'});
  //  $(this.tableId+'_tab0').setStyle({width:($(this.tableId+'_tab0').getWidth()-this.options.scrollBarWidth)+'px'});
  },
  ...
  remainingHt: function() {
    ...
    var margin=Prototype.Browser.IE ? 50 : 40;
    switch (this.sizeTo) {
      case 'window':
      case 'data':
        ...
        return winHt-divPos[1]-tabHt-this.options.scrollBarWidth-60;//margin;  // allow for scrollbar and some margin
      case 'body':
  ...
  pluginWindowResize: function() {
    ...
    this.resizeWindowHandler2=this.resizeWindow2.bindAsEventListener(this);
    Event.observe(window, "resize", this.resizeWindowHandler, false);
if($('SpaceIndex_tab'))
{
    calcSpaceIndexWidth();
    Event.observe('splitPanelimg', "click", this.resizeWindowHandler2, false);
}
  },
...
if($('SpaceIndex_tab'))
    Event.stopObserving('splitPanelimg', "click", this.resizeWindowHandler2, false);
