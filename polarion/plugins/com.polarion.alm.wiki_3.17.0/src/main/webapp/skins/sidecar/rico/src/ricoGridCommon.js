...
Rico.GridCommon.prototype = {
  ...
  baseInit: function() {
    this.options = {
      resizeBackground : 'blank.gif',
      saveColumnInfo   : {width:false, filter:false, sort:false},  // save info in cookies?
      allowColResize   : true,      // allow user to resize columns
      windowResize     : false,      // Resize grid on window.resize event? Set to false when embedded in an accordian.
      click            : null,
      dblclick         : null,
      contextmenu      : null,
...
//    alert($(pageHeightId).value);
	...
	this.scrTabWi = this.scrTabWi-this.options.scrollBarWidth+10;
...
Rico.TableColumn.MINWIDTH   = 50; // min column width when user is resizing
 ...
 */
  baseInit: function(liveGrid,colIdx,hdrInfo,tabIdx) {
    ...
    this.hdrCellDiv=(divs.length<2) ? RicoUtil.wrapChildren(this.hdrColDiv,'ricoLG_cell ricoLG_cellPointer') : divs[1];
 ...
 */
  ...
  handleMouseUp: function(e) {
    ...
    var pageWidthId = this.liveGrid.tableId.replace("itemTable","itemTableWidth");
    if($(pageWidthId))
    var pwidth = $(pageWidthId).value;
	else
    var pwidth = "100%";
if(document.all){
    $(this.liveGrid.tableId+'_outerDiv').setStyle({overflow:'auto',width:pwidth+'px'});
    $(this.liveGrid.tableId+'_tab0').setStyle({width:(parseInt(pwidth)-19)+'px'});
    if(parseInt($(this.liveGrid.tableId+'_innerDiv').getWidth())<parseInt(pwidth))
    $(this.liveGrid.tableId+'_scrollDiv').setStyle({width:(parseInt(pwidth))+'px'});
}else{
    $(this.liveGrid.tableId+'_outerDiv').setStyle({width:$(pageWidthId).value+'px'});
    $(this.liveGrid.tableId+'_scrollDiv').setStyle({width:$(pageWidthId).value+'px'});
//    $(this.liveGrid.tableId+'_innerDiv').setStyle({width:(parseInt($(pageWidthId).value)-19)+'px'});
    $(this.liveGrid.tableId+'_tab0').setStyle({width:(parseInt($(pageWidthId).value)-38)+'px'});
}		
    Event.stop(e);
  },
