WikiEditor.prototype.initCorePlugin = function() {
    ...
    // sc - 452 borovik
    this.addInternalProcessor((/&#91;/i), '\\\[');                                                  
    this.addInternalProcessor((/&#93;/i), '\\\]');
    this.addExternalProcessor((/\\\[/i), '\\&#91;');
    this.addExternalProcessor((/\\\]/i), '\\&#93;');
    ...
    this.addExternalProcessorBefore('convertTableExternal', (/\{style:type=div\s*(.*?)\}([\s\S]+?)\{style\}/i), 'convertStyleExternal');
    this.addExternalProcessorBefore('convertTableExternal', (/\{style:type=span\s*(.*?)\}([\s\S]+?)\{style\}/i), 'convertStyleExternal');
    this.addExternalProcessorBefore('convertTableExternal', (/\{style:type=font\s*(.*?)\}([\s\S]+?)\{style\}/i), 'convertStyleExternal');

    
	// External/Internal conversion setup
	this.addInternalProcessor((/<\/pre><pre>/gi), '<br />');
...
WikiEditor.prototype.getStyleControl = function(button_name) {
    switch(button_name) {
        ...
        case "fontselect":
            ...
            var iFonts = 'Arial=Arial;Courier New=Courier New;Georgia=Georgia;Tahoma=Tahoma;Times New Roman=Times New Roman;Verdana=Verdana;Impact=Impact;WingDings=Wingdings';
            var nFonts = 'Andale Mono=Andale Mono;Arial=Arial;Arial Black=Arial Black;Book Antiqua=Book Antiqua;Comic Sans MS=Comic Sans MS;Courier New=Courier New;Georgia=Georgia;Helvetica=Helvetica;Impact=Impact;Symbol=Symbol;Tahoma=Tahoma;Terminal=Terminal;Times New Roman=Times New Roman;Trebuchet MS=Trebuchet MS;Verdana=Verdana;Webdings=Webdings;Wingdings=Wingdings';
...
WikiEditor.prototype.PARAGRAPH_CLASS_NAME = "";
...
WikiEditor.prototype.convertListExternal = function(regexp, result, content) {
    ...
    //var subContent = this._convertNewLine2BrInList(subContent);
    var str = "";
...
WikiEditor.prototype._convertRecursiveListExternal = function(regexp, content, depth, classname) {
	...
	subContent = this.trimString(subContent);
	...
	if(currdepth > 0) {
        str += "<li>" + this.trimString(r[2]); //+ "<\/li>\r\n";
    switch (subContent.charAt(1)) {
		case '*':
			classname =  this.LIST_NORMAL_CLASS_NAME;
			break;
        case '-':
			classname =  this.LIST_MINUS_CLASS_NAME;
			break;
        case '#':
            classname =  this.LIST_NUMERIC_CLASS_NAME;
			break;
        case '1':
            classname =  this.LIST_NUMERIC_CLASS_NAME;
            break;
...
WikiEditor.prototype.convertListInternal = function(regexp, result, content) {
	...
	if(bounds && bounds["start"] > -1) {
        ...
        str = this.convertListInternalNew(content.substring(bounds["start"], bounds["end"]), lclass);
            ...
            if (attributes["style"]) {
                ...
                for (var i=0; i < atts.length ; i++) {
                    ...
                    for (var j=0 ; j < styleAtts.length; j++) {
                        if (att == styleAtts[j])
                        {
                            if (att == "font-family" && value.indexOf(" ") != -1 && value.indexOf("'") == -1 )
                               str += "|" + att + "=" + "'"+value+"'";
                            else
                               str += "|" + att + "=" + value;
                            break;
...
WikiEditor.prototype.convertListInternalNew = function(content, lclass)
{
	var str = "";
	...
	content = content.replace(/<ul\s*([^>]*)\s*>/gi, "");
	content = content.replace(/<\/ul>/gi, "");
	
	var result;
	var list_regexp = /<li\s*([^>]*)\s*>/i;
	
	while( (result = content.match(list_regexp)) ) 
	{
			...
			switch(attributes["wikieditorlisttype"])
			{
				case 'ul':
                    if ((lclass != null) && (lclass == "minus")) {
                        str_1 += this.buildString("-", parseInt(attributes["wikieditorlistdepth"], 10)) + " ";
                    } else {
                        str_1 += this.buildString("*", parseInt(attributes["wikieditorlistdepth"], 10)) + " ";
                    }
					str_1 = str_1.replace(/\n/gi, " ");
					break;
				case 'ol':
						str_1 = this.buildString("1", parseInt(attributes["wikieditorlistdepth"], 10)) + ". ";
						str_1 = str_1.replace(/\n/gi, " ");
                    break;
			}
		}		
		//alert(result[0]+ "   "+str_1);
		content = content.replace(result[0], "\r\n" + str_1);
	}
	
	content = content.replace(/\n/gi, " ");
	
	// replace space 11.[space] 111. text	
	list_regexp = /(1+)\.\s*(<br \/>)*\s*(1+)\./i;
	while( (result = content.match(list_regexp)) ) 
	{
		result = content.match(list_regexp);
		var n = result[0].lastIndexOf(" ");
		var s ="";
		if (n != -1)
			s = result[0].substring(n, result[0].length);
			s = this.trimString(s);
		//alert(result);
		content = content.replace(result[0], s);
	}
	
	// replace space **.[space] ***. text	
	list_regexp = /(\*+)\s+(<br \/>)*\s*(\*+)/i;
	while( (result = content.match(list_regexp)) ) 
	{
		result = content.match(list_regexp);
		var n = result[0].lastIndexOf(" ");
